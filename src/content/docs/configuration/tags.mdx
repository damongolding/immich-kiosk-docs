---
title: Tags
---

import { Aside } from '@astrojs/starlight/components';
import Settings from "../../../components/config-settings.astro";

## Getting a tag value from Immich
1. Open Immich's web interface and click on "Tags" in the left-hand navigation.
2. Click on the tag you want the value of.
3. The url will now look something like this `http://192.168.86.123:2283/tags?path=cake`.
4. The tag value is everything after `path=`, so in this example it would be `cake`.

## Tags

A date range or ranges. See Date range for more information.

<Settings
    config="tags"
    env="KIOSK_TAGS"
    url="tag"
    value="[]string"
    defaultValue="[]"
    configOver={`tags:
    - TAG_VALUE
    - TAG_VALUE
    `}
    envOver={`environment:
    KIOSK_TAGS: "TAG_VALUE, TAG_VALUE, TAG_VALUE"
    `}
    urlOver={"http://{URL}?tag=TAG_VALUE&tag=TAG_VALUE&tag=TAG_VALUE"}
/>

### How multiple tags work
When you specify multiple tags, Immich Kiosk creates a pool of all the requested tag values.
For each asset refresh, Kiosk randomly selects one of the tag values from this pool and fetches an asset associated with it.

---

## Excluded Tags

You can use this feature to hide assets with certain tags from appearing in the slideshow.

<Settings
    config="excluded_tags"
    env="KIOSK_EXCLUDED_TAGS"
    url="exclude_tag"
    value="[]string"
    defaultValue="[]"
    configOver={`excluded_tags:
    - TAG_VALUE
    - TAG_VALUE
    `}
    envOver={`environment:
    KIOSK_EXCLUDED_TAGS: "TAG_VALUE, TAG_VALUE, TAG_VALUE"
    `}
    urlOver={"http://{URL}?exclude_tag=TAG_VALUE&exclude_tag=TAG_VALUE&exclude_tag=TAG_VALUE"}
/>

---

## Special tags

### Skip
Tagging an asset with "kiosk-skip" will exclude it from displaying.


## Tag wildcards
You can use wildcard patterns when including or excluding tags:

- `parent` - Match exactly "parent"
- `parent/*` - Match direct children only (e.g., "parent/child")  
- `parent/**` - Match all descendants (e.g., "parent/child/grandchild")

Example configuration:
```yaml
tags:
  - vacation
  
excluded_tags:
  - vacation/2023/**  # Exclude all 2023 vacation photos
```
