...
## Docker
    ...
    You will need to mount a volume into the container to store the downloaded assets.
    ...
    Kiosk does not run as root, so if you see permission errors you may need to adjust the mounted directory's permissions.

    ```bash
    sudo chown -R 65532:65532 /path/to/offline-assets
    ```
